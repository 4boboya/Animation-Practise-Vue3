...
import { GetTopAdv, EditAdv, CreateAdv, } from "@/apis/BackendService";
...
export default defineComponent({
  ...
  setup() {
    ...
    const getAdvertising = async () => {
      ...
      setReqData();
      await GetTopAdv(state.reqData).then((res: any) => {
    ...
    const setFormData = async () => {
      ...
      formData.append("Seq", (state.updateData.seq as number).toString());
    ...
    watch(
      ...
      () => {
        if (state.updateData.seq as number > 99) {
          state.updateData.seq = 99
        } else if (state.updateData.seq as number < 1) {
          state.updateData.seq = null
        }
        if (state.updateData.seq as number % 1 != 0){
          state.updateData.seq = Math.floor(state.updateData.seq as number)
        }
      },
      { deep: true }
    )
